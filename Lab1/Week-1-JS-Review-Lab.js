 ...
 */
//WRITE YOUR CODE BELOW
console.log("Task 3 → template literal check:");
...
const realtorFees = (listing) => {
  const priceNum = Number(String(listing.price).replace(/[\$,]/g, ""));
  // Number() conversion: https://www.w3schools.com/jsref/jsref_number.
  //https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/RegExp
  // Regex replace removes $ and commas https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/replace
  const fee = priceNum <= 450000 ? priceNum * 0.025 : priceNum * 0.02;
  // Ternary operator: https://www.w3schools.com/js/js_comparisons.asp#ternary
  return Math.round(fee);
};
console.log("Task 4 → realtorFees:");
console.log("Listing[2] fee:", realtorFees(listings[2]));
console.log("Listing[6] fee:", realtorFees(listings[6]));
/**
 * Task-5: Sort the listing array ascendingly in a new variable called listingAscendingly
 */
//WRITE YOUR CODE BELOW
const listingAscendingly = [...listings].sort((numA, numB) => {
  // Spread: https://www.w3schools.com/react/react_es6_spread.asp
  // sort()  https://www.w3schools.com/js/js_array_sort.asp
  //https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/RegExp
  const priceA = Number(String(numA.price).replace(/[\$,]/g, ""));
  const priceB = Number(String(numB.price).replace(/[\$,]/g, ""));
  return priceA - priceB; 
});
console.log("Task 5 → sorted listings by price:");
listingAscendingly.forEach((l) => console.log(`${l.address} → ${l.price}`));
/**
 ...
 */
//WRITE YOUR CODE BELOW
const newBuiltListings = listings.filter((l) => {
  let isValid = false;
  const builtIn = l.propertySummary && l.propertySummary.builtIn 
  ? l.propertySummary.builtIn 
  : "";
  const matchYear = builtIn.match(/\b(19|20)\d{2}\b/); 
  // regex extracts a 4-digit year
  if (matchYear) {
    const year = parseInt(matchYear[0], 10); // convert string to number
    if (year >= 1990) {
      isValid = true;
    }
  }
...
console.log("Task 6 → newBuiltListings (built >= 1990):");
newBuiltListings.forEach((l) => console.log(`${l.address} → built in ${l.propertySummary.builtIn}`));